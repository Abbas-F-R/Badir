...
) : IUserService
{
    ...
    {
        var user = await wrapper.User.Get(u => u.Id == update.Id,
        ...
        var permission = await wrapper.Permission.GetById(update.PermissionId);
        if (permission == null)
        {
            return (null, "الاذن غير موجود");
        }
    ...
    public async Task<(UserDto?, string? error)> RemovePermission(UserPermissionsUpdate update)
    {
        var user = await wrapper.User.Get(u => u.Id == update.Id,
            x => x.Include(p => p.Permission!));
        if (user == null)
        ...
        var permissionToRemove = user.Permission!.FirstOrDefault(p => p.Id == update.PermissionId);
        if (permissionToRemove == null)
        {
            return (null, "الاذن غير موجود");
        }

        user.Permission!.Remove(permissionToRemove);
        var result = await wrapper.User.Update(user);
        return (mapper.Map<UserDto>(result), null);
    }
